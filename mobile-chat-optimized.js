class MobileChatOptimized {
    ...
    setupFirebase() {
        if (window.firebaseDB) {
            // Utiliser la même référence que le PC
    ...
    setupEventListeners() {
        ...
        // Envoyer message
        document.getElementById('sendMobileChatBtn').addEventListener('click', () => {
            this.sendMessage();
        });
    ...
    sendMessage() {
        ...
        this.chatRef.push(messageData).then(() => {
            console.log('✅ Message envoyé depuis mobile');
            input.value = '';
        }).catch(error => {
    ...
    getCurrentUser() {
        if (window.userManager && window.userManager.currentUser) {
            return {
                uid: window.userManager.currentUser.uid,
                nickname: window.userManager.currentUser.nickname || 'Mobile User',
                email: window.userManager.currentUser.email
            };
        }
        
        return {
            uid: 'mobile_user_fixed',
...
document.addEventListener('DOMContentLoaded', () => {
    window.mobileChatOptimized = new MobileChatOptimized();
});